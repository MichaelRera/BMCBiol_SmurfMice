...
C57_F_bp <- read.csv("BMC/me_used_and_bp.csv")
C57_F_bp <- C57_F_bp %>% transform(me_used_c57_f = as.character(me_used_c57_f))
C57_F_bp <- C57_F_bp %>% dplyr::select(variable,me_used_c57_f,c57_f_no_me,c57_f_me)
C57_F_bp <- C57_F_bp %>% dplyr::mutate(BP = case_match(me_used_c57_f, "FALSE" ~ c57_f_no_me, "TRUE" ~ c57_f_me)) %>% select(variable,BP)
selection <- c("BW", "BT", "FAT", "FINKCALs", "GLY", "P5")
temp_bp <- C57_F_bp %>% filter(variable %in% selection)
temp_bp <- temp_bp %>% mutate(BP = ifelse(variable == "FAT", -61.99, BP)) %>% 
  mutate(BP = ifelse(variable == "GLY", -24.1, BP))
...
#C57_F_data_lessNA <- C57_F_data %>% filter(!is.na(RERNm))
...
temp1a <- C57_F_data %>% dplyr::select(mice, date_expe, AGE, AGE_MONTHS, REMAINTIME, PRED, BW, BT, FAT, FINKCALs, GLY, P5)
...
write.csv(temp1a_pca1,"BMC/C57_F_data_NS_S.csv", na = "NA")
