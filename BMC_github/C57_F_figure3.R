...
C57_F_fig3 <- data_fig3 %>%
  filter(grepl("^C57_F_", mice_name))

C57_F_y_min <- 0
C57_F_y_max <- max(C57_F_fig3$life_expectency)

C57_F_plot1 <- ggplot(C57_F_fig3, aes(x = "", y = life_expectency)) +
  ...
  ylim(C57_F_y_min, C57_F_y_max)+
  theme_minimal()+
  theme(axis.line = element_line(color = "grey70"),
        axis.title = element_text(face = "bold"))

C57_F_stats_plot1 <- C57_F_fig3 %>%
...
print(C57_F_stats_plot1)

filtered_data <- C57_F_fig3 %>% filter(life_expectency > 0 & life_as_smurf > 0)
...
C57_F_plot2 <- ggplot(C57_F_fig3, aes(x = "", y = life_as_smurf)) +
...
print(C57_F_stats_plot2)


#plot 3&4 C57_F
C57_F_data <- read_csv("BMC/C57_F_data.csv")
C57_F_data <- C57_F_data %>% mutate(REMAINTIME = abs(REMAINTIME)) %>% mutate(PRED = ifelse(REMAINTIME>60,"under60","above60"))

#add age in months
C57_F_data <- C57_F_data %>% mutate(AGE_MONTHS = AGE/30.5)
C57_F_data$AGE_MONTHS <- style_number(C57_F_data$AGE_MONTHS, digits = 1)

#select variables for PCA
temp1a_C57_F <- C57_F_data %>% dplyr::select(mice, date_expe, AGE, AGE_MONTHS, REMAINTIME, PRED, BW, BT, FAT, FINKCALs, GLY, P5)

#impute missing values to run PCA
temp1b_C57_F <- temp1a_C57_F %>% dplyr::select(!c(mice, date_expe, AGE, AGE_MONTHS, REMAINTIME,PRED))
nb1 <- estim_ncpPCA(temp1b_C57_F,ncp.max=6, scale = FALSE)
nb1
imputed_temp1a_C57_F <- imputePCA(temp1a_C57_F, ncp = 4, scale = TRUE, quali.sup = c(1, 2, 3, 4, 5, 6))

#run and plot PCA
res.pca1_C57_F = PCA(imputed_temp1a_C57_F$completeObs, scale.unit= TRUE, ncp=4, graph=T, quali.sup = c(1, 2, 3, 4, 5, 6))

#classification of individuals into Smurfs and Non-Smurfs based on PCA dimension 1 values
pca1_C57_F <- get_pca_ind(res.pca1_C57_F)
temp1a_pca1_C57_F <- bind_cols(temp1a_C57_F,pca1_C57_F$coord) %>% dplyr::select(!c(Dim.2, Dim.3, Dim.4))
pca1_C57_F_threshold <- temp1a_pca1_C57_F %>% group_by(PRED) %>% dplyr::summarise(mean = mean(Dim.1, na.rm = TRUE), sd= sd(Dim.1, na.rm = TRUE)) %>% 
  filter(PRED == "under60") 
value_pca1_C57_F_threshold <- pca1_C57_F_threshold$mean-(pca1_C57_F_threshold$sd*1.65)
value_pca1_C57_F_threshold
temp1a_pca1_C57_F <- temp1a_pca1_C57_F %>% mutate(STATUS = if_else(Dim.1>value_pca1_C57_F_threshold,"NS","S"))

#C57 plot3
temp1a_pca1_C57_F_plot3 <- temp1a_pca1_C57_F
temp1a_pca1_C57_F_plot3 <- temp1a_pca1_C57_F_plot3 %>%
  mutate(AGE_MONTHS = factor(AGE_MONTHS, levels = unique(AGE_MONTHS)))
significance_results <- temp1a_pca1_C57_F_plot3 %>%
  group_by(AGE_MONTHS) %>%
...
C57_F_plot3 <- temp1a_pca1_C57_F_plot3 %>%
  ...
  theme(axis.text.x = element_text(size = 7),
        ...
        axis.title = element_text(face = "bold")) +
  scale_x_discrete(breaks = levels(temp1a_pca1_C57_F_plot3$AGE_MONTHS)[seq(1, length(levels(temp1a_pca1_C57_F_plot3$AGE_MONTHS)), by = 3)])+
...
#C57 plot4
temp1a_pca1_C57_F_plot4 <- temp1a_pca1_C57_F %>% dplyr::group_by(AGE_MONTHS, STATUS) %>% dplyr::summarise(n = n()) %>% dplyr::mutate(PROP = (n/sum(n))*100) 
temp1a_pca1_C57_F_plot4 <- temp1a_pca1_C57_F_plot4 %>% transform(AGE_MONTHS = as.numeric(AGE_MONTHS))
data <- temp1a_pca1_C57_F_plot4 %>% dplyr::filter(STATUS == "S")
...
C57_F_plot4 <- ggplot(data, aes(x = AGE_MONTHS, y = PROP)) +
