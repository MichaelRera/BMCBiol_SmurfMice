...
run_pipeline(var_names[[1]], data = data_c57_f, use_me = FALSE, x_var = "AGE")
###
y_var = "GLY"
data <- data_c57_f
use_me <- FALSE
...
x_var = "AGE"
###
run_pipeline(y_var, data = data, use_me = FALSE, x_var = "AGE")

slm_tot <- lapply(var_names, run_pipeline, data_c57_f, use_me = FALSE,
                  file_path = file.path("figure", data_dir, "c57_wrt_age/female/c57_f_"),
                  x_var = "AGE")
system(paste0("cd figure/", data_dir, "/c57_wrt_age/female/ && pdftk c57* cat output whole_c57_f.pdf"))
...
plot(data_c57_f$AGE, data_c57_f$BW)
